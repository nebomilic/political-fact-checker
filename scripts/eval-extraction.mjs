...
//
// CLI wrapper around `vitest run` for the extraction eval. Vitest itself
...
//
// Usage:
//   npm run eval:extraction                       # OpenAI (default)
//   npm run eval:extraction -- --provider mistral
//   npm run eval:extraction -- --compare           # both, side by side
...
for (let i = 0; i < args.length; i++) {
	...
	if (arg === "--compare") {
		...
		console.error(
			"Usage: npm run eval:extraction -- [--provider openai|mistral] [--compare]",
		);
		process.exit(1);
	}
}
...
const env = {
	...process.env,
	EVAL_EXTRACTION_PROVIDER: provider,
	EVAL_EXTRACTION_COMPARE: compare ? "1" : "",
};

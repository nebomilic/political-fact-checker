...
- Voice input (Quick Check's mic button, `src/routes/quick.tsx`) is recorded client-side (`MediaRecorder`/`getUserMedia`) and transcribed server-side via OpenAI's Whisper API — not persisted anywhere, only forwarded to OpenAI. Replaced the original client-only Web Speech API approach, which proved unreliable outside actual Google Chrome (see ADR 0007). See `SCOPE.md`'s Quick Check bullet

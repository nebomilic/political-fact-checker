...
- Voice input (Quick Check's mic button, `src/routes/quick.tsx`) is transcribed client-side via the browser's Web Speech API only — no server-side STT, no audio upload/storage, no new provider dependency. See `SCOPE.md`'s Quick Check bullet and ADR 0006
